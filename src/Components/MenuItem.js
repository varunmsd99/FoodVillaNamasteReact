...
const MenuItem = ({name, description, imageId, defaultPrice, price, isVeg, variants, variantsV2}) => {
  ...
  const handleAddItem = () => {
    dispatch(addItems({name, description, imageId}));
  }
    return(
        <div className="flex justify-between items-center border-b-solid border-b-[0.5px] border-b-[#d3d3d3] my-5 pb-3 w-full last:border-b-0">
          ...
          <div className="flex flex-col items-center  ">
            ...
            <button className="relative w-24 h-9 bottom-2 text-[#60b246] bg-white cursor-pointer rounded text-sm font-bold border-[1.11px] border-solid border-gray-300 shadow-sm transform -translate-y-1/2 z-1 hover:shadow-md" onClick={handleAddItem}>ADD</button>
          </div> 
        </div>
    )
}
