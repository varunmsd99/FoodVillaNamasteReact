...
const Body = () => {
  ...
  [footCities, setFootCities] = useState([]);
  ...
  async function getRestaurants() {
    ...
    const footCitiesData = citiesFooterData[0].splice(6);
  ...
  console.log(footCities);
  return (
    resList.length === 0 ? <div>Shimmer</div> : <>
    <div className="max-w-[80%] mx-auto min-h-screen pt-24">
      {topicalBanner && (
        <>
          <div className="mt-6 ml-4 pl-2">
          ...
          <div className="flex overflow-hidden">
            {topicalBanner.map((img) => {
              return (
                <div className="ml-6 mt-8 cursor-pointer" key={img.imageId}>
                  <img
        ...
        <div className="grid grid-cols-4 gap-x-[5%] w-[80%] mx-auto">
          <div>
            <div className="flex items-center" key="footer-details">
              <img src={foodVillaLogoWhite} className="h-16" key={foodVillaLogoWhite}/>
              <h1 className="font-BasisGrotesquePro font-extrabold text-2xl leading-10 tracking-tighter text-white">Food Villa</h1>
            </div>
            <h1>&nbsp;&nbsp;&nbsp;© 2023 Bundl Technologies Pvt. Ltd</h1>
          </div>
          {footer_data.map((x, index) => {
            return (
              <div className={`${index === 2 ? 'col-start-3 -mt-32' : ''}`} key={index}>
                <h1 className="font-BasisGrotesquePro font-extrabold text-base leading-10 tracking-tighter text-white">{x.title}</h1>
                {[...x.data].map((k, index) => {
                  return(
                    <div key={`${index+k}`}>
          ...
          <div className="col-start-4 row-start-1">
            <h1 className="font-BasisGrotesquePro font-extrabold text-base leading-10 tracking-tighter text-white">We deliver to:</h1>
            {citiesFooter.map((x) => {
              return (
                <div key={x.text}>
            ...
            <div className="flex mt-6 justify-between align-center items-center cursor-pointer border-[#ffffff26] border-solid border-[1px] w-fit px-2 rounded-xl" onClick={toggleCitites}>
              <h1 className="p-1">{footCities.length} cities &nbsp;</h1>
              <div className="ml-2 pr-1">{<FontAwesomeIcon icon={faChevronDown} className={cityOpen ? 'rotate-180':''}/>}</div>
              </div>
          </div>
        </div>
      </div>
      <div className={cityOpen ? 'flex flex-col items-center justify-center pt-6 pb-10 text-[#ffffff99] bg-[#02060c] w-full h-fit border-t-[#ffffff26] border-solid border-t-[1px]' : 'hidden'}>
